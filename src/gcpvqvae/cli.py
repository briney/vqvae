...
from gcpvqvae.data.mmcif import write_mmcif
...
def encode_cmd() -> None:
    ...
    parser.add_argument("input_cif", type=Path, help="Path to the input mmCIF file.")
    ...
    print(f"Encoding {args.input_cif} chain {args.chain}...")
    max_len = cfg.get('data', {}).get('length_cap', 2048)
    result = model.encode(str(args.input_cif), args.chain, max_length=max_len)

    if result is None:
        print(f"Error: Could not process {args.input_cif} chain {args.chain}.", file=sys.stderr)
        sys.exit(1)
...
def decode_cmd() -> None:
    ...
    parser.add_argument("--out", type=Path, required=True, help="Path to the output .cif file.")
    parser.add_argument("--checkpoint", type=Path, required=True, help="Path to the model checkpoint.")
    parser.add_argument("--device", default="cpu", help="Device to run the model on (e.g., 'cuda:0').")
    args = parser.parse_args()
    ...
    print(f"Successfully saved decoded structure to {args.out}")
